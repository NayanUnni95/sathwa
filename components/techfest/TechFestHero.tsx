...
export default function TechFestHero() {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            const runAnimation = (targetOuter: number, targetInner: number, hugeOuter: number, hugeInner: number) => {
                ...
                gsap.set('.joystick-layer', { scale: 0.5, opacity: 0, filter: 'blur(10px)', y: 0 });
                ...
                tl.add(() => { gsap.set('.layer-2, .layer-3', { display: 'block' }); }, ">-0.1");

                tl.to('.layer-1', { y: -hugeOuter, duration: 1.2 }, "expand1")
                    .to('.layer-4', { y: hugeOuter, duration: 1.2 }, "expand1")
                    .to('.layer-2', { y: -hugeInner, opacity: 1, scale: 1, filter: 'blur(0px)', duration: 1.2 }, "expand1") // Fade in internals
                    .to('.layer-3', { y: hugeInner, opacity: 1, scale: 1, filter: 'blur(0px)', duration: 1.2 }, "expand1");
                ...
                tl.to('.layer-1', { y: -smallOuter, duration: 0.6, ease: "sine.out" })
                    .to('.layer-2', { y: -smallInner, duration: 0.6, ease: "sine.out" }, "<")
                    .to('.layer-3', { y: smallInner, duration: 0.6, ease: "sine.out" }, "<")
                    .to('.layer-4', { y: smallOuter, duration: 0.6, ease: "sine.out" }, "<");

                // Step F: Final Settle (Target)
                tl.to('.layer-1', { y: -targetOuter, duration: 0.8, ease: "elastic.out(1, 0.5)" })
                    .to('.layer-2', { y: -targetInner, duration: 0.8, ease: "elastic.out(1, 0.5)" }, "<")
                    .to('.layer-3', { y: targetInner, duration: 0.8, ease: "elastic.out(1, 0.5)" }, "<")
                    .to('.layer-4', { y: targetOuter, duration: 0.8, ease: "elastic.out(1, 0.5)" }, "<");
    ...
    return (
        <section ref={containerRef} className="relative w-full h-screen bg-[#050505] overflow-hidden flex flex-col items-center justify-center font-sans text-white selection:bg-[#BC002D] selection:text-white">
            ...
            <div className="relative z-30 w-full h-[80vh] flex items-center justify-center mt-10 pointer-events-none">
                <div className="relative w-[300px] md:w-[450px] aspect-[3/4]">
                    ...
                    <div className="joystick-layer layer-4 absolute inset-0 z-0 drop-shadow-2xl opacity-0">
                        <Image src="/assets/homepage/layer-4.webp" alt="Joystick Layer 4" fill className="object-contain" priority />
                    </div>
                    <div className="joystick-layer layer-3 absolute inset-0 z-10 drop-shadow-xl opacity-0">
                        <Image src="/assets/homepage/layer-3.webp" alt="Joystick Layer 3" fill className="object-contain" priority />
                    </div>
                    <div className="joystick-layer layer-2 absolute inset-0 z-20 drop-shadow-lg opacity-0">
                        <Image src="/assets/homepage/layer-2.webp" alt="Joystick Layer 2" fill className="object-contain" priority />
                    </div>
                    <div className="joystick-layer layer-1 absolute inset-0 z-30 drop-shadow-2xl opacity-0">
                        <Image src="/assets/homepage/layer-1.webp" alt="Joystick Layer 1" fill className="object-contain" priority />
                    </div>
                </div>
            </div>
            ...
            <div className="absolute bottom-10 right-6 md:right-12 z-40 ui-reveal">
                <div
                    ...
                    style={{ clipPath: 'polygon(15% 0, 100% 0, 100% 100%, 0 100%, 0 45%)' }}
                >
