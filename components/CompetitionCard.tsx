...
export default function CompetitionCard({ id, title, description, date, day, imageUrl = "/assets/dummy2.jpg", details = false }: CompetitionCardProps) {
            ...
            <div className="flex flex-col flex-grow relative bg-[#050505]">
                ...
                <div className="flex w-full border-b border-white/10">
                    ...
                    <div className="w-20 md:w-24 flex-shrink-0 flex flex-col items-center justify-center border-r border-white/10 bg-[#0a0a0a] py-4 group-hover:bg-[#BC002D] group-hover:text-white transition-colors duration-300">
                        <span className="text-2xl md:text-3xl font-['KyivTypeTitling'] font-bold leading-none">{dayNum}</span>
                        <span className="text-[9px] md:text-[10px] tracking-widest uppercase mt-1 opacity-80">{monthName}</span>
                    </div>

                    {/* Title Area */}
                    <div className="flex-grow p-4 md:p-5 flex flex-col justify-center">
                        <span className="text-[8px] md:text-[9px] tracking-[0.2em] uppercase text-[#BC002D] mb-1 group-hover:text-[#BC002D] transition-colors">Competition</span>
                    ...
                    {details && (
                        <div className="grid grid-cols-2 divide-x divide-white/10 border-t border-white/10">
                            ...
                            <div className="p-3 flex flex-col items-center">
                                <span className="text-[8px] uppercase tracking-widest text-zinc-600 mb-1">Time</span>
                                <span className="text-xs font-mono text-zinc-300 uppercase">10:30 AM</span>
                            </div>


                            <div className="p-3 flex flex-col items-center">
                                <span className="text-[8px] uppercase tracking-widest text-zinc-600 mb-1">Venue</span>
                                <span className="text-xs font-mono text-zinc-300 uppercase">Seminar Hall</span>
                            </div>
                        </div>
                    )}
