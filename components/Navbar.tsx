  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 flex items-center justify-between px-6 py-4 md:px-12 md:py-8">
      ...
      >
        ...
        ></span>
        <span
          className={`block w-4 h-0.5 bg-black ml-auto transition-all duration-300 ${isOpen ? "-rotate-45 -translate-y-2 w-6" : ""}`}
        ></span>
      </button>

      {/* Mobile/Overlay Menu */}
      <div
        className={`fixed inset-0 bg-white/95 flex flex-col items-center justify-center space-y-8 transition-transform duration-500 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <Link
          href="/"
